...
namespace QES_Connect
{
    public partial class Connect_Control : UserControl
    {
        ...
        public static Label _fileName_label;
        ...
        private void ProcessingData()
        {
            try
            {
                ...
                if (fileWrite == true) // (Backup_Control._indices_btn_bool == true)
                {
                    ...
                    //MyTxtFile.Write(fileString);
                    //MyCSVFile.Write(fileString);
                    //MyXLSFile.Write(fileString);

                    //Stream fs = null;

                    //var sw = new StreamWriter(fs, Encoding.UTF8);
                    //sw = fileString.Replace("**", "\u2103");


                    //string str1 = str.Replace("\u00B0", "hey");

                    //Unicode (hex) of ° is 00B0
                    //ASCII (decimal) of ° is 176

                    //using (var writer = new StreamWriter(fs, false, Encoding.GetEncoding("Windows-1252"))
                    //{ }
                    // MyCSVFile.Write(str);


                    //MyTxtFile.Write(str1);
                    //MyCSVFile.Write(str1);
                    //MyXLSFile.Write(str1);

                    str = fileString.Replace("*C", "" + "" + "\x02DA" + "");
                    str = str.Replace("SavedData", null);
                    str = str.Trim();
                    // replace *C by unicode char in case of writing only to csv file
                    ...
                    //MyCSVFile.OutputEncoding = Encoding.Unicode;
